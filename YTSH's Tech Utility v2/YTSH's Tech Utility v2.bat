...
if "!SAFE_MODE!"=="1" (
    powershell -NoProfile -Command "Write-Host '    v2.0  | ADMINISTRATOR  | SAFE MODE: ENABLED (Advanced features locked)' -ForegroundColor !COL_HDR_PS!"
) else (
    powershell -NoProfile -Command "Write-Host '    v2.0  | ADMINISTRATOR  | SAFE MODE: DISABLED (Full access)' -ForegroundColor Yellow"
)
powershell -NoProfile -Command "Write-Host '  =============================================' -ForegroundColor !COL_HDR_PS!"
echo.
...
(
    ...
    echo try {
    echo     Checkpoint-Computer -Description 'Before YTSH Debloat Utility v2.0' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop
...
(
    echo Enable-ComputerRestore -Drive 'C:\' -ErrorAction SilentlyContinue
    echo try { Checkpoint-Computer -Description 'Before YTSH Debloat v2.0 Single Op' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop } catch {}
) > "%RPSCRIPT%"
...
shutdown /r /t 5 /c "YTSH Debloat Utility v2.0: Applying changes."
exit /b 0
