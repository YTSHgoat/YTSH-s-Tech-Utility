...
if not exist "%SystemRoot%\MEMORY.DMP" set /a CLEAN_CNT+=1
...
powershell -NoProfile -Command "Write-Host '  -----------------------------------------------' -ForegroundColor !COL_HDR_PS!"
echo.
powershell -NoProfile -Command "Write-Host '   A  ^>  Apply ALL Network Optimizations' -ForegroundColor Yellow"
echo   B  ^>  Back to main menu
echo.
powershell -NoProfile -Command "Write-Host '  =============================================' -ForegroundColor !COL_HDR_PS!"
echo.
...
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" /v NetworkThrottlingIndex /t REG_DWORD /d 4294967295 /f >nul 2>&1
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" /v SystemResponsiveness /t REG_DWORD /d 0 /f >nul 2>&1
powershell -NoProfile -Command "Write-Host '  [SUCCESS] Network Throttling Disabled (SystemProfile tuned)' -ForegroundColor Green"
exit /b 0
...
if "!DCHOICE!"=="2"  set "DPKG=Microsoft.BingNews"                     & goto DO_SINGLE_APP
...
if "!RCHOICE!"=="2"  set "RPKG=Microsoft.BingNews" & goto REVERT_SINGLE_APP
...
(
    ...
    echo $disk = Get-WmiObject -Class MSStorageDriver_FailurePredictStatus -Namespace root\wmi -ErrorAction SilentlyContinue
    ...
    echo $mem = Get-WmiObject Win32_PhysicalMemory -ErrorAction SilentlyContinue
    ...
    echo $gpu = Get-WmiObject Win32_VideoController ^| Where-Object { $_.ConfigManagerErrorCode -ne 0 }
    ...
    echo $mobo = Get-WmiObject Win32_BaseBoard ^| Where-Object { $_.Status -ne 'OK' }
    ...
    echo $badDrivers = Get-WmiObject Win32_PnPEntity -ErrorAction SilentlyContinue ^| Where-Object { $_.ConfigManagerErrorCode -ne 0 -and $_.ConfigManagerErrorCode -ne $null }
    echo if ^($badDrivers^) {
...
(
    ...
    echo     $bstat  = if ^($bat.BatteryStatus -eq 1^) { 'Discharging' } elseif ^($bat.BatteryStatus -eq 2^) { 'On AC ^(not charging^)' } ...{ 'Charging' } else { 'Status ' + $bat.BatteryStatus }
...
:: Logic: If RAM < 8GB, set pagefile to 1.5x RAM. If > 8GB, set to 4GB fixed.
...
wmic pagefileset where name="C:\\pagefile.sys" set InitialSize=%PF_SIZE%,MaximumSize=%PF_SIZE% >nul 2>&1
