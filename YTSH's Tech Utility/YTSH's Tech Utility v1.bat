...
(
    ...
    echo $sys = Get-CimInstance Win32_ComputerSystem   -ErrorAction SilentlyContinue
    echo $osName = if ^($os^) { ^($os.Caption -replace 'Microsoft ', ''^).Trim^(^) } else { 'Unknown OS' }
    echo $out += "SYS_OS=$osName"
    echo $chassis = Get-CimInstance Win32_SystemEnclosure -ErrorAction SilentlyContinue
    echo $laptopTypes = @^(8,9,10,11,12,14,18,21,30,31,32^)
    echo $formFactor = 'DESKTOP'
    echo if ^($chassis^) { foreach ^($ct in $chassis.ChassisTypes^) { if ^($laptopTypes -contains [int]$ct^) { $formFactor = 'LAPTOP'; break } } }
    echo $out += "SYS_FORM=$formFactor"
    echo $mfr = if ^($sys^) { $sys.Manufacturer.Trim^(^) } else { '' }
    echo $brand = switch -Wildcard ^($mfr.ToUpper^(^)^) { 'HP*' { 'HP' } 'HEWLETT*' { 'HP' } 'DELL*' { 'Dell' } 'LENOVO*' { 'Lenovo' } 'ASUS*' { 'ASUS' } 'ACER*' { 'Acer' } 'MSI*' { 'MSI' } 'TOSHIBA*' { 'Toshiba' } 'SAMSUNG*' { 'Samsung' } 'MICROSOFT*' { 'Microsoft' } 'RAZER*' { 'Razer' } 'HUAWEI*' { 'Huawei' } 'LG*' { 'LG' } default { if ^($mfr -ne ''^) { $mfr } else { 'Unknown' } } }
    echo $out += "SYS_BRAND=$brand"
    echo $cpu = Get-CimInstance Win32_Processor -ErrorAction SilentlyContinue ^| Select-Object -First 1
    echo $cpuName  = if ^($cpu^) { ^($cpu.Name.Trim^(^) -replace '\^(R^\)','' -replace '\^(TM^\)','' -replace '  ',' '^).Trim^(^) } else { 'Unknown CPU' }
    echo $cpuCores = if ^($cpu^) { [string]$cpu.NumberOfCores } else { '?' }
    echo $cpuGHz   = if ^($cpu^) { [string]^([math]::Round^($cpu.MaxClockSpeed / 1000.0, 1^)^) + ' GHz' } else { '' }
    echo $out += "SYS_CPU=$cpuName"
    echo $out += "SYS_CPU_CORES=$cpuCores"
    echo $out += "SYS_CPU_GHZ=$cpuGHz"
    echo $totalRamGB = if ^($os^) { [math]::Round^($os.TotalVisibleMemorySize / 1MB, 0^) } else { 0 }
    echo $out += "SYS_RAM=$totalRamGB GB"
    ...
    echo $gpu = $gpus ^| Select-Object -First 1
    echo $gpuName = if ^($gpu^) { ^($gpu.Name.Trim^(^) -replace '\^(R^\)','' -replace '\^(TM^\)',''^).Trim^(^) } else { 'Unknown GPU' }
    echo $out += "SYS_GPU=$gpuName"
    echo if ^($gpus.Count -gt 1^) { $gpu2 = ^($gpus ^| Select-Object -Skip 1 -First 1^).Name.Trim^(^) -replace '\^(R^\)','' -replace '\^(TM^\)','' ; $out += "SYS_GPU2=$gpu2" } else { $out += 'SYS_GPU2=' }
    echo $cDrive = Get-PSDrive -Name C -PSProvider FileSystem -ErrorAction SilentlyContinue
    echo $diskTotalGB = if ^($cDrive^) { [math]::Round^(^[math]::Max^($cDrive.Used + $cDrive.Free, 1^) / 1GB, 0^) } else { 0 }
    echo $diskFreeGB  = if ^($cDrive^) { [math]::Round^($cDrive.Free / 1GB, 1^) } else { 0 }
    echo $diskType = 'HDD'
    ...
    echo $out += "SYS_DISK_TYPE=$diskType"
    echo $out += "SYS_DISK_TOTAL=$diskTotalGB GB"
    echo $out += "SYS_DISK_FREE=$diskFreeGB GB"
    echo $out ^| Set-Content -Path "$env:TEMP\ytsh_sysinfo.txt" -Encoding ASCII
) > "%DETECT_SCRIPT%"
...
(
    ...
    echo Write-Host ^('  Unsigned              : ' + $unsigned.Count^) -ForegroundColor ^(if^($unsigned.Count -gt 0^){'Red'}else{'Green'}^)
    echo Write-Host ^('  Potentially outdated  : ' + $old.Count^) -ForegroundColor ^(if^($old.Count -gt 0^){'Yellow'}else{'Green'}^)
) > "%DRVSCRIPT%"
